...
# Install cri-o into release stage.
make install -e DESTDIR=${destdir}

# Copy crictl tool into release stage
cp ${GOPATH}/bin/crictl ${destdir}/usr/local/bin/
# Install systemd units and config into release stage.
make install.config -e DESTDIR=${destdir}
make install.systemd -e DESTDIR=${destdir}
make install.completions -e DESTDIR=${destdir}
make install.policy -e DESTDIR=${destdir}
